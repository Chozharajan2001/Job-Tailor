...
beforeAll(async () => {
  ...
  const user = await User.create({
    ...
    passwordHash:
      "$2b$10$abcdefghijklmnopqrstuuABCDEFGHIJKLMNOPQRSTUVWXYZ012345", // valid bcrypt-length stub
    firstName: "Test",
    lastName: "Workflow",
  });
...
describe("8. Application Outcome Recording (Sprint 5)", () => {
  it("should set callbackReceived and push a callback timeline event", async () => {
    ...
    expect(
      updated!.timelineEvents.some((e) => e.event === "Callback received"),
    ).toBe(true);
  });
  ...
  it("should record rejectedReason and push a rejection note timeline event", async () => {
    ...
    expect(
      updated!.timelineEvents.some(
        (e) => e.event === "Rejection reason recorded",
      ),
    ).toBe(true);
  });
  ...
  it("should record offerAmount and push an offer note timeline event", async () => {
    ...
    expect(
      updated!.timelineEvents.some((e) => e.event === "Offer details recorded"),
    ).toBe(true);
  });
});
...
describe("9. Reminder Completion (Sprint 5)", () => {
  it("should mark a reminder complete and push a timeline event", async () => {
    ...
    expect(
      updated!.timelineEvents.some((e) => e.event === "Reminder completed"),
    ).toBe(true);
  });
  ...
  it("should not overwrite completedAt on a second completion", async () => {
    ...
    expect(refetched!.reminders[0].completedAt!.getTime()).toBe(
