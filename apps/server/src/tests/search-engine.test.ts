    ...
    <script type="application/ld+json">
      {
        ...
        "url": "https://93.184.216.34/jobs/staff-eng-123"
      }
    </script>
  </head>
  <body>
...
describe("Advanced Job Search Engine (Sprint 2) Integration Suite", () => {
  ...
  describe("1. Ingestion Pipeline & Contracts", () => {
    it("should successfully ingest manual copy-paste jobs and parse properties", async () => {
      const job = await IngestionService.ingestFromPaste({
        ...
        jdRawText:
          "We are seeking a Senior Full Stack Engineer. Experience with React, Node, and TypeScript is required. Please apply today!",
  ...
  describe("2. Deduplication Layer", () => {
    it("should deduplicate exact URL imports (L1)", async () => {
      ...
      const job1 = await IngestionService.ingestFromPaste({
        ...
        jdRawText:
          "We are hiring a DevOps Engineer. Experience with AWS and Terraform is required.",
        location: "Seattle, WA",
      });
      ...
      expect(job2.lastSeenAt.getTime()).toBeGreaterThanOrEqual(
        job1.lastSeenAt.getTime(),
      );
    });
    ...
    it("should deduplicate by description text hash similarity (L3)", async () => {
      ...
      const desc =
        "We are hiring a Python Engineer for data scraping. Pandas, BeautifulSoup, Selenium experience needed.";
  ...
  describe("3. Global Search & Relevance Ranking (Sprint 2)", () => {
    beforeAll(async () => {
      ...
      await IngestionService.ingestFromPaste({
        ...
        jdRawText:
          "Build UI apps. Requires deep knowledge of TypeScript and CSS.",
        location: "Remote",
      });
    ...
    it("should boost relevance score for jobs matching profile skills (personalization)", async () => {
      const searchRes = await SearchService.searchJobs({
        q: "Engineer",
        userId: mockUserId,
      });

      const reactJob = searchRes.jobs.find(
        (j) =>
          j.companyName === "Hype Technologies" && j.jobTitle === "UI Engineer",
      );
      const railsJob = searchRes.jobs.find(
        (j) =>
          j.companyName === "Hype Technologies" &&
          j.jobTitle === "Backend Engineer",
      );
    ...
    it("should filter search results independently using advanced filters", async () => {
      ...
      const result2 = await SearchService.searchJobs({
        employmentType: "full-time",
      });
  ...
  describe("4. Saved Searches CRUD & Alerts Dispatch", () => {
    ...
    it("should generate an Alert when a newly ingested job matches a SavedSearch", async () => {
      ...
      const alertsAfterNonMatch = await Alert.find({
        userId: mockUserId,
      }).exec();
      ...
      expect(alertsAfterMatch[0].canonicalJobId.toString()).toBe(
    ...
    it("should list alerts and mark an alert as read via controller functions", async () => {
      ...
      const { listAlerts, markAlertAsRead } =
        await import("../controllers/alert.controller.js");
  ...
  describe("5. Stale Job Cleanup Job", () => {
    it("should mark stale canonical jobs inactive and exclude them from search results", async () => {
      ...
      const activeJob = await IngestionService.ingestFromPaste({
        ...
        jdRawText:
          "Ruby on Rails and Postgres application developer job description.",
        location: "Remote",
      });
  ...
  describe("6. Watches, Curated Feeds & Link Verification (Sprint 3)", () => {
    it("should support Watch CRUD and trigger alerts for matching watches on ingestion", async () => {
      ...
      expect(alerts[0].canonicalJobId.toString()).toBe(
        matchingJob._id.toString(),
      );
      ...
      await new Promise((resolve) => setTimeout(resolve, 300));
      const alertsAfterDisable = await Alert.find({
        userId: mockUserId,
      }).exec();
    ...
    it("should generate curated feeds and exclude imported jobs", async () => {
      ...
      const feedResultAfterImport =
        await FeedService.getPersonalizedFeed(mockUserId);
      const containsVercel = feedResultAfterImport.feed.some(
        (item) => item.companyName === "Vercel",
      );
  ...
  describe("7. Ingestion Quality, Trust Decay & Search Analytics (Sprint 4)", () => {
    it("should support verificationState defaults and update states on link checks", async () => {
      ...
      const job = await CanonicalJob.create({
        ...
        applyUrl: "https://93.184.216.34/jobs/404-check",
    ...
    it("should log user click interactions and handle flagging feedback to update states/trust", async () => {
      ...
      const clickLogs = await JobInteractionLog.find({
      ...
      const spamLogs = await JobInteractionLog.find({
