...
describe("Profile Education CRUD & Concurrency Tests", () => {
  it("should successfully perform full Education CRUD operations", async () => {
    ...
    const updatedProfileEdit = await profileService.updateEducation(
      testUserId,
      eduId,
      {
        degree: "Ph.D.",
    ...
    const editedEdu = updatedProfileEdit!.education.find(
    ...
    // 4. Delete Education
    const updatedProfileDel = await profileService.deleteEducation(
      testUserId,
      eduId,
    );
  ...
  it("should successfully perform full Certification CRUD operations", async () => {
    ...
    // 2. Add Certification
    const updatedProfileAdd = await profileService.addCertification(
      testUserId,
      {
    ...
    const updatedProfileEdit = await profileService.updateCertification(
      testUserId,
      certId,
      {
    ...
    expect(updatedProfileEdit).not.toBeNull();
    const editedCert = updatedProfileEdit!.certifications.find(
    ...
    const updatedProfileDel = await profileService.deleteCertification(
      testUserId,
      certId,
    );
  ...
  it("should support concurrent subdocument writes without VersionError or array overwrites", async () => {
    ...
    const freshProfile = await Profile.findOne({ userId: testUserId })
      .lean()
      .exec();
