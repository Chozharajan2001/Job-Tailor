...
import {
  CanonicalJob,
  ICanonicalJobDocument,
} from "../models/CanonicalJob.model.js";
...
export class IngestionService {
   ...
   */
  static async ingestJob(
    input: IJobIngestionInput,
    ...
    if (!workType) {
      ...
      if (locationLower.includes("hybrid") || descLower.includes("hybrid")) {
        ...
        descLower.includes("office only") ||
        descLower.includes("work from office")
      ) {
        workType = "onsite";
      } else {
        workType = "remote";
    ...
    if (input.sourceType === "manual_paste") {
      ...
      if (sourceUrl) {
        try {
          ...
          source = await SourceRegistry.findOne({
            baseUrl: { $regex: host, $options: "i" },
            isEnabled: true,
          });
        } catch (e) {
    ...
    // 3. Derive dedupe key
    const dedupeKey = DeduplicationService.generateDedupeKey(
      companyName,
      jobTitle,
      location,
    );
    ...
    const descriptionHash =
      DeduplicationService.generateDescriptionHash(description);
  ...
  static async ingestFromPaste(params: {
    ...
    try {
      ...
      console.warn(
        "AI structured parse failed, falling back to simple mapping:",
        e,
      );
    }
  ...
  static async ingestFromUrl(urlStr: string): Promise<ICanonicalJobDocument> {
    ...
    // Fetch raw HTML through the SSRF-safe fetcher (scheme/IP validation,
    // redirect re-validation, timeout, and 2 MB body cap)
    const { body: html } = await safeFetchText(urlStr);
    ...
    let employmentType: "full-time" | "part-time" | "contract" | "internship" =
      "full-time";
    ...
    if (jobPosting) {
      ...
      if (jobPosting.employmentType) {
        ...
        else if (typeStr.includes("contract") || typeStr.includes("temp"))
      ...
      const metaSiteName = extractMeta(
