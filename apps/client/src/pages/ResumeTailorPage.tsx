...
export default function ResumeTailorPage() {
  ...
  // Distinct cache key from the Jobs page list (different payload shape)
  const { data: jobsRes } = useQuery({
    queryKey: queryKeys.jobs.picker(100),
    queryFn: () => api.get<{ jobs: IJob[] }>("/jobs", { limit: 100, page: 1 }),
  });
  ...
  const { data: resumesRes, isLoading: loadingResumes } = useQuery({
    queryKey: queryKeys.resumes.byJob(selectedJobId || null),
    queryFn: () =>
      api.get<{ resumes: IResume[] }>(`/resumes?jobId=${selectedJobId}`),
  ...
  const generateMutation = useMutation({
    mutationFn: () =>
      api.post<{ resume: IResume }>("/resumes/generate", {
        jobId: selectedJobId,
      }),
    onSuccess: () => {
  ...
  const updateResumeMutation = useMutation({
    mutationFn: async ({ id, body }: { id: string; body: any }) => {
      const response = await api.put<{ resume: IResume }>(
        `/resumes/${id}`,
        body,
      );
    ...
    onSuccess: () => {
      // Invalidate the exact key the query above uses (was a dead key before)
      queryClient.invalidateQueries({
        queryKey: queryKeys.resumes.byJob(selectedJobId || null),
      });
    ...
    onError: (error: any) => {
      alert(
        error.response?.data?.error?.message ||
          "Failed to update resume summary",
      );
    },
  });
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Select a parsed job to auto-generate a tailored resume with ATS
          scoring.
        </p>
      </div>
            ...
            {generateMutation.isError && (
              <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-sm text-red-700 space-y-2">
                ...
                <p className="text-xs text-red-600">
                  Make sure your master profile has details (Skills, Experience,
                  Summary) set up.
                </p>
                <div className="pt-1">
                  <Link
                    to="/profile"
                    className="text-xs px-3 py-1.5 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 inline-block transition-colors"
                  >
                    Go to Profile Setup →
                  </Link>
                </div>
            ...
            {existingResumes.length > 0 && (
              <div className="space-y-3">
                <h3 className="font-semibold text-sm uppercase tracking-wide text-muted-foreground">
                  Generated Versions ({existingResumes.length})
                </h3>
                {existingResumes.map((resume) => (
                  <ResumeCard
                    ...
                    onUpdateResume={(body) =>
                      updateResumeMutation.mutate({ id: resume._id, body })
                    }
...
function ResumeCard({
  ...
  return (
    <div
      className={`border rounded-xl overflow-hidden transition-all bg-white ${expanded ? "shadow-md border-primary/20" : "hover:border-slate-300"}`}
    >
      <div
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-4 border-t pt-3 bg-slate-50/20">
          ...
          <div>
            ...
            {isEditing ? (
              ...
              <p className="text-xs text-slate-700 leading-relaxed bg-slate-50/50 p-3 rounded-lg border border-slate-100">
                {resume.tailoredSummary || "No summary generated."}
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <ScoreBar
              label="Keyword Match"
              value={s?.keywordMatchScore || 0}
              color="#22c55e"
            />
            <ScoreBar
              label="Semantic"
              value={s?.semanticMatchScore || 0}
              color="#a855f7"
            />
            <ScoreBar
              label="Sections"
              value={s?.sectionCompletenessScore || 0}
              color="#3b82f6"
            />
            <ScoreBar
              label="Format"
              value={s?.formatScore || 0}
              color="#f97316"
            />
          </div>
        </div>
...
function ATSScoreBadge({ score }: { score: number }) {
  ...
  return (
    <span
      className={`px-3 py-1.5 rounded-lg text-sm font-bold border ${color}`}
    >
      ATS: {score}/100
    </span>
  );
  ...
  return (
    <div className="border rounded-xl p-5 space-y-5 shadow-sm bg-white">
      ...
      <div className="flex items-center gap-6">
        <div
          className={`relative w-28 h-28 rounded-full flex items-center justify-center ${scoreGaugeColor(score.overallScore)}`}
        >
          <svg
        ...
        <div className="space-y-1.5 flex-1">
          ...
          <ScoreBar
            label="Keywords"
            value={score.keywordMatchScore}
            color="#22c55e"
            showValue
          />
          <ScoreBar
            label="Semantic"
            value={score.semanticMatchScore}
            color="#a855f7"
            showValue
          />
          <ScoreBar
      ...
      {breakdown.matchedSkills.length > 0 && (
        <section>
          <h4 className="text-xs font-semibold uppercase tracking-wide text-green-600 mb-2">
            ✅ Matched Skills (
            {breakdown.matchedSkills.filter((s) => s.presentInResume).length}/
          ...
          <div className="flex flex-wrap gap-1.5">
            {breakdown.matchedSkills.map((ms) => (
              <span
                key={ms.skill}
                title={`${ms.presentInResume ? "✓ In resume" : "✗ Not in resume"} · JD requires this`}
        ...
        <section>
          ...
          <div className="space-y-1.5">
            {breakdown.missingSkills.map((ms) => (
              <div
                key={ms.skill}
                className="flex items-start gap-2 text-sm bg-red-50/50 p-2 rounded-lg"
              >
                <span className="font-medium text-red-700 mt-0.5">
                  {ms.skill}
                </span>
                {ms.required && (
        ...
        <section>
          ...
          <div className="space-y-1.5">
            {breakdown.weakSkills.map((ws) => (
              <div
                key={ws.skill}
                className="flex items-start gap-2 text-sm bg-orange-50/50 p-2 rounded-lg"
              >
                <span className="font-medium text-orange-700 mt-0.5">
                  {ws.skill}
                </span>
                <span className="text-[10px] px-1.5 py-0.5 bg-orange-100 text-orange-600 rounded shrink-0">
                  {ws.userYearsExp}y vs {ws.requiredYearsExp}y needed (-{ws.gap}
                  y)
                </span>
        ...
        <section>
          ...
          <ul className="space-y-1.5">
            {breakdown.actionItems.map((item, i) => (
              <li
                key={i}
                className="text-sm flex items-start gap-2 bg-blue-50/50 p-2 rounded-lg"
              >
