...
// Local alias kept to minimize churn in component prop types below
type DashboardData = AnalyticsOverview;
...
export default function AnalyticsPage() {
  ...
  const funnelData = d?.pipelineFunnel
    ? Object.entries(d.pipelineFunnel).map(([stage, count]) => ({
  ...
  const performanceData = (d?.resumePerformance || []).map((p) => ({
    name:
      p.versionLabel.length > 25
        ? p.versionLabel.slice(0, 22) + "..."
        : p.versionLabel,
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Learn from your application data to improve your job search.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {[
          ...
          },
          {
            ...
            sub: `${Math.round((d?.interviewRate || 0) * (d?.totalApplications ?? 0))} reached interview`,
          },
          {
            label: "Offer Rate",
            value: `${Math.round((d?.offerRate || 0) * 100)}%`,
            sub: `${Math.round((d?.offerRate || 0) * (d?.totalApplications ?? 0))} offers received`,
          },
          {
            label: "Avg ATS Score",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border p-6 shadow-sm">
          <h2 className="font-semibold mb-4 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-green-500"></span> Top
            Matching Skills
          </h2>
          {d?.topMatchingSkills && d.topMatchingSkills.length > 0 ? (
              ...
              {(() => {
                const maxSkillCount = Math.max(
                  ...d.topMatchingSkills.map((item) =>
                    typeof item === "string" ? 1 : item.count,
                  ),
                  1,
                );
                return d.topMatchingSkills.map((item, i) => {
                  ...
                  const pct =
                    maxSkillCount > 0
                      ? Math.max(15, (count / maxSkillCount) * 100)
                      : 0;
                  return (
                    <div key={skill} className="flex items-center gap-3">
                      <span
                        className="w-24 text-sm font-medium truncate text-gray-700"
                        title={skill}
                      >
                        {skill}
                      </span>
            ...
            <p className="text-center text-sm text-muted-foreground py-8">
              Generate resumes to see skill matching data.
            </p>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border p-6 shadow-sm">
          <h2 className="font-semibold mb-4 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-red-500"></span> Common
            Skill Gaps
          </h2>
          {d?.commonGaps && d.commonGaps.length > 0 ? (
            <div className="space-y-2.5">
              {(() => {
                ...
                return d.commonGaps.map((item, i) => {
                  ...
                  const pct =
                    maxGapCount > 0
                      ? Math.max(15, (count / maxGapCount) * 100)
                      : 0;
                  return (
                    <div key={gap} className="flex items-center gap-3">
                      <span
                        className="w-24 text-sm font-medium truncate text-red-700"
                        title={gap}
                      >
                        {gap}
                      </span>
            ...
            <p className="text-center text-sm text-muted-foreground py-8">
              No gaps detected — your profile covers most JDs!
            </p>
          )}
        </div>
      </div>
...
function PerformanceChart({
  ...
  data: Array<{ name: string; usageCount: number; callbackRate: number }>;
}) {
  if (!data.length) {
    return (
      <div className="bg-white rounded-xl border p-6 shadow-sm">
        ...
        <p className="text-center text-sm text-muted-foreground py-12">
          Generate multiple resume versions to compare performance.
        </p>
      </div>
    );
  }
        ...
        {data.map((item) => (
          ...
          >
            <div className="flex items-start justify-between mb-2">
              ...
              <span
                className={`text-sm font-bold px-2 py-0.5 rounded ${item.callbackRate >= 20 ? "bg-green-50 text-green-700" : item.callbackRate >= 10 ? "bg-yellow-50 text-yellow-700" : "bg-gray-50 text-gray-600"}`}
              >
    ...
    <div className="bg-gradient-to-r from-primary/5 to-indigo-50 rounded-xl border border-primary/10 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {insights.map((insight, i) => (
          <div
            key={i}
            className="flex items-start gap-3 bg-white/70 rounded-lg p-3 border border-white"
          >
...
function generateInsights(
    ...
    return [
      {
        ...
        detail:
          "Add jobs and generate resumes to unlock personalized insights.",
      },
    ];
  ...
  if (rate >= 0.3)
    insights.push({
      ...
      detail: `Your ${(rate * 100).toFixed(0)}% interview rate is above average. Your tailored resumes are working!`,
    });
  else if (rate > 0)
    insights.push({
      icon: "⚡",
      title: "Boost Your Interview Rate",
      detail: `At ${(rate * 100).toFixed(0)}%, focus on improving ATS scores above 80 for better results.`,
    });
  else if (data.totalApplications > 0)
    insights.push({
      icon: "🚀",
      title: "Get Started",
      detail: `You have ${data.totalApplications} applications tracked. Generate resumes to start getting interviews.`,
    });

  if ((data.averageATSScore || 0) < 75)
    insights.push({
      icon: "📈",
      title: "Improve ATS Scores",
      detail: `Average score is ${data.averageATSScore}/100. Add missing skills and quantify achievements with metrics.`,
    });

  if ((data.thisWeekApplied || 0) < 3)
    insights.push({
      icon: "📝",
      title: "Apply More This Week",
      detail: `Only ${data.thisWeekApplied} applications this week. Target 5+ per week to increase your chances.`,
    });

  if (data.commonGaps && data.commonGaps.length > 0) {
    const gapNames = data.commonGaps
      .slice(0, 3)
      .map((g) => (typeof g === "string" ? g : g.skill));
    insights.push({
      icon: "💪",
      title: "Close Your Skill Gaps",
      detail: `Focus on learning: ${gapNames.join(", ")}. These appear in multiple JDs.`,
    });
  }
  ...
  if (insights.length === 0)
    insights.push({
      ...
      title: "Looking Good!",
      detail:
        "Your metrics are healthy. Keep applying consistently and track results.",
    });
